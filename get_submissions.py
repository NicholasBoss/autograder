...
def fetch_submissions(api_url=None, token=None, course_id=None, assignment_id=None, data_dir=None, downloads_root=None, manifest_path=None):
    # Use parameters if provided, otherwise fall back to environment variables
    api_url = api_url or CANVAS_API_URL
    token = token or CANVAS_API_TOKEN
    course_id = course_id or CANVAS_COURSE_ID
    assignment_id = assignment_id or CANVAS_ASSIGNMENT_ID
    data_dir = data_dir or DATA_DIR
    downloads_root = downloads_root or DOWNLOAD_DIR
    ...
    base = api_url.rstrip('/') + '/'
    submissions_url = urljoin(base, f"api/v1/courses/{course_id}/assignments/{assignment_id}/submissions")
    params = {
    ...
    # Create helper function with local headers
    def _get_paginated_local(url, params=None):
        params = params or {}
        items = []
        next_url = url
        while next_url:
            r = requests.get(next_url, headers=session_headers, params=params, timeout=30)
            if r.status_code != 200:
                raise RuntimeError(f"Canvas API error: {r.status_code} - {r.text}")
            page_items = r.json()
            if isinstance(page_items, list):
    ...
    items = _get_paginated_local(submissions_url, params=params)
    
    # Ensure download directory exists
    os.makedirs(downloads_root, exist_ok=True)
    
    # Use provided manifest_path or create default
    ...
    for s in items:
        ...
        for a in attachments:
            ...
            saved_path = os.path.join(downloads_root, safe_name)
            ok = False
            err = None
            if download_url:
                # Use local download function with correct headers
                try:
                    r = requests.get(download_url, stream=True, headers=session_headers, timeout=60)
                    if r.status_code == 200:
    ...
    return {"students": len(rows), "files_downloaded": downloaded, "manifest": manifest_path}
